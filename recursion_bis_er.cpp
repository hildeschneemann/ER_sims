...
void recursion(int Nv, int nbSv, int NbGenv, int pasv, double s, double s_max, double I, double U_g, double U_c, double U_t, double Rg, double Rc, int Rep, int output, double** allAverages, double sdopt, double steep)
{
    ...
    stringstream nomF2;
    nomF2 << "geno_N" << Nv << "_nbS" << nbSv << "_nbGen" << NbGenv << "_s" << s << "_I" << I  << "_Ug" << U_g << "_Uc" << U_c << "_Ut" << U_t<< "_Rg" << Rg << "_Rc" << Rc << "_sdopt" << sdopt << "_steep" << steep << "_Rep" << Rep << ".txt"; // results file naming convention
    nomF2 >> nomFichier2;

    char nomFichier[256];
    stringstream nomF;
    nomF << "N" << Nv << "_nbS" << nbSv << "_nbGen" << NbGenv << "_s" << s << "_I" << I  << "_Ug" << U_g << "_Uc" << U_c << "_Ut" << U_t<< "_Rg" << Rg << "_Rc" << Rc << "_sdopt" << sdopt << "_steep" << steep << "_Rep" << Rep << ".txt"; // results file naming convention
	...
	fout << "gen\twbar\tmismatch\thomfreq\thetfreq\n";
...
//            dg = s; // CONSTANT: -s fitness mutation
        ...
        {
            w = getfitness(pop[i], expdom, s_max, Q0, I, nbSv, sdopt, steep);
            Wtot[i] = w;
            if (wmax < w)
                wmax = w;
        }
        ...
        if (gen % pasv == 0) // "gen" is the generational index (iterable) and once it reaches a multiple of the time interval per generation "pasv", write results
        {
            ...
            {
                record_averages_seq(pop, Wtot, popAverages, nbSv, Nv, expdom, s_max, Q0, I, sdopt, steep);
		...
		for ( i = 0; i < Nv; i++)
		{ 
			fout2 << Wtot[i] << "," << pop[i].gene[0] << "," << pop[i].gene[1] << ",";
			for (j = 0; j < nbSv; j++)
			{
				fout2 << pop[i].cis[j] << "," << pop[i].cis[nbSv + j] << ", " << pop[i].trans[j] << "," << pop[i].trans[nbSv + j] << ",";
			}
		}
		fout2 << endl;
		...
		for (j = 0; j < Nv; j++)
		{
			...
			} while (rnd.rand() > (Wtot[p1] / wmax));
            // second parent:

			do{
                p2 = rnd.randInt(N_1); // select random parent 2

            } while (rnd.rand() > (Wtot[p2] / wmax) | p1 == p2);
